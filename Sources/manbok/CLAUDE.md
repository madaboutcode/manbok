...
| Invocation | Behavior |
|------------|----------|
| `manbok authorize\|start\|stop\|status\|dump\|sessions` | `CommandRouter` → mic TCC or `UnixSocketClient` |
| `manbok daemon` (hidden) | `DaemonMain.runDaemon()` — do not run manually unless debugging |
