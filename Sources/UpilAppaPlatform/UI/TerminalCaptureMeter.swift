...
public final class TerminalCaptureMeter: ActivityPresenting, @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "ai.upil.appa.meter", qos: .userInitiated)
    ...
    private func render(
        ...
        switch mode {
        case .watching:
            lines.append(ANSIColor.wrap(ANSIColor.cyan + ANSIColor.bold, "◌ WATCHING  upil-appa"))
        ...
        lines.append(ANSIColor.wrap(ANSIColor.dim, "  logs → Console (ai.upil.appa) · IPC: status | dump | stop"))
