...
/// Ported verbatim from Sources/ManbokApp/Views/Theme.swift (production source of truth)
/// so the spike stops drifting. `bgRoom` at the bottom is the one spike-only addition
/// (backdrop for the interactive vibe-check window) — production has no equivalent
/// because the real popover never renders outside the menu bar chrome.
enum Theme {
    // MARK: Surfaces
    ...
    // MARK: Spike-only — interactive vibe-check window backdrop
...
/// it turns only while `spinning`. Callers format the counter text.
